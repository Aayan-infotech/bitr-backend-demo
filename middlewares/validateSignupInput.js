...
function validateSignupInput(req, res, next) {
    ...
    if (["user", "mentor", "instructor", "prisoner"].includes(safeRole)) {
        ...
        if (
            ...
            !/^[A-Za-z]+(?: [A-Za-z]+)*$/.test(trimmedName)
        ) {
            return res.status(400).json({
                status: "error",
    ...
    if (["user"].includes(safeRole)) {
        if (!safeNumber || !/^\+?\d{1,25}$/.test(safeNumber)) {
            return res.status(400).json({
                status: "error",
                message: "Number must contain only digits, can start with '+', and be up to 25 characters.",
            });
        }
    }
    ...
    // if (["user", "mentor"].includes(safeRole)) {
    //     if (!safeDOB || !/^\d{2}[-/]\d{2}[-/]\d{4}$/.test(safeDOB)) {
    //         return res.status(400).json({
    //             status: "error",
    //             message:
